...
export const aiProviderOptions: Array<{ value: AiProviderId; label: string; models: string[] }> = [
  ...
  { value: 'lmstudio', label: 'LM Studio', models: ['local-model', 'qwen2.5-7b-instruct', 'llama-3.2-3b-instruct'] },
...
export function readLmStudioSettings() {
  ...
  try {
    const parsed = JSON.parse(storedValue) as Partial<typeof defaultLmStudioSettings>;
...
export function saveLmStudioSettings(settings: Partial<typeof defaultLmStudioSettings>) {
  window.localStorage.setItem(lmStudioSettingsStorageKey, JSON.stringify(normalizeLmStudioSettings(settings)));
  window.dispatchEvent(new Event('mwananchi-lm-studio-settings'));
}

export function withLocalProviderSettings(selection: AiModelSelection): AiModelSelection {
...
export function normalizeAiSelection(selection: AiModelSelection): AiModelSelection {
  ...
  return {
    provider,
    model,
    ...(provider === 'lmstudio' && selection.baseUrl ? { baseUrl: selection.baseUrl } : {}),
  };
}

function normalizeLmStudioSettings(settings: Partial<typeof defaultLmStudioSettings>) {
  return {
    ...
    models: Array.isArray(settings.models)
      ? [...new Set(settings.models.map((model) => model.trim()).filter(Boolean))]

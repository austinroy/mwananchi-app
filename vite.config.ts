...
const tesseractCoreDirectory = dirname(require.resolve('tesseract.js-core/package.json'));
...
function tesseractAssetsPlugin() {
  const coreFiles = readdirSync(tesseractCoreDirectory).filter((fileName) => {
    ...
    return fileName.startsWith('tesseract-core') && (extension === '.js' || extension === '.wasm');
  });

  return {
    name: 'mwananchi-tesseract-assets',
    configureServer(server) {
      server.middlewares.use('/ocr/tesseract-worker/worker.min.js', (_request, response) => {
        response.setHeader('content-type', 'text/javascript');
        response.end(readFileSync(tesseractWorkerPath));
      });

      server.middlewares.use('/ocr/tesseract-core/', (request, response, next) => {

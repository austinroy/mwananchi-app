...
Netlify routes `/api/*` to `netlify/functions/api.mjs` before the SPA fallback. The function uses Turso over HTTP for production storage and now covers the frontend-facing API routes for user sync, AI defaults, encrypted AI keys, provider model lists, briefs, shared briefs, chat messages, civic action drafts, visibility updates, and deletes. Brief/chat/action AI generation in the Netlify function currently returns prototype responses; full provider-backed generation still lives in `server/index.mjs` and should be moved next.

...
Offline work is queued in the browser when the API is unreachable. Brief creation, chat messages, action drafts, visibility changes, and deletes are saved to an encrypted IndexedDB-backed local queue and retried when the browser comes back online. The API remains the primary source of truth; offline storage is only a temporary client-side sync layer. When Clerk is enabled, the local encryption key is derived from the active Clerk user/session token. Local development and guest sessions use a browser-local fallback key.

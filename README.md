...
The app uses the installed `i18n` package with static English (`en`), Kiswahili (`sw`), Arabic (`ar`), French (`fr`), and Portuguese (`pt`) dictionaries. Users can change the UI language from the navigation menu, and the choice is saved in browser `localStorage`. The selected language is also sent with AI brief, chat, and action-generation requests so new generated content follows the UI language.

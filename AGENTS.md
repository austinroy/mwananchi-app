...
- Frontend API requests use `normalizeApiBaseUrl` in `src/lib/apiBaseUrl.ts` so local browser runs default to `http://localhost:8787`, deployed hosts without `VITE_API_BASE_URL` use same-origin `/api/*`, and host-only deploy values are converted to HTTPS instead of becoming relative app paths.
- Netlify routes `/api/*` to `netlify/functions/api.mjs` before the SPA fallback. The function uses Turso over HTTP and covers frontend-facing routes for user sync, AI defaults, encrypted AI keys, provider model lists, briefs, shared briefs, chat messages, civic action drafts, visibility updates, and deletes. Brief/chat/action AI generation in Netlify currently returns prototype responses; full provider-backed generation still lives in `server/index.mjs` and should be moved next.
...
- When no AI provider is selected, user-facing notices should ask the user to set one up in Account. If a provider is selected but no key is available, keep the provider-specific missing-key notice.

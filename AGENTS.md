...
- Netlify routes `/api/*` to `netlify/functions/api.mjs` before the SPA fallback. That function currently supports health checks, user sync, and user AI default reads/writes with Turso over HTTP; move the remaining `server/index.mjs` API routes into the serverless handler before relying on Netlify for the complete production API.

...
- Netlify routes `/api/*` to `netlify/functions/api.mjs` before the SPA fallback. That function is currently a scaffold with health/CORS handling; move the full `server/index.mjs` API routes into a reusable function handler and use durable hosted storage before relying on Netlify for the production API.

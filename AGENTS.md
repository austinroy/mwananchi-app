...
- Offline work uses an encrypted browser-side IndexedDB queue/cache in `src/lib/offlineStore.ts`. The API remains primary storage; offline data is temporary and syncs through `syncOfflineChanges()` in `src/lib/api.ts` when the browser regains network access. Clerk-enabled sessions derive offline encryption from an authenticated per-user offline wrapping key issued by the API and stored encrypted server-side with `API_KEY_ENCRYPTION_SECRET`; local development and guests use a browser-local fallback key.
